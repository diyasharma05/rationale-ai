...
| **Someone asks "who approved that message?"** | Audit request | Append-only ledger and outbox: draft, approval, send are separate events with actor and time. On PostgreSQL the application role is INSERT-only, so the log cannot be edited by the app | Export to the client's audit system |
